...
const CardExpert = ({ index, imgProfile, name, position, detail, imgLogo }) => {
  ...
  return (
    <div className={`w-96 h-5/6 m-8 p-8 ${bgColor} flex flex-col justify-between rounded-md`}>
      <Image onError={(e) => e.target.style.display = 'none'} className="w-32 h-32 rounded-full mb-4" src={imgProfile} alt={name} width={128} height={128} />
      ...
      <div className="flex m-5 justify-center">
        <Image onError={(e) => e.target.style.display = 'none'} width={150} height={150} className="rounded-sm" src={imgLogo} alt="Logo" />
      </div>
    </div>
  );
}
